...
export default function FloatingAd() {
  ...
  const trackImpression = async (adId: string) => {
    try {
      await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL || 'https://api.listup.ng/api'}/advertisements/${adId}/impression`
      );
    } catch (error) {
  ...
  const handleAdClick = async () => {
    ...
    try {
      // Track click
      await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL || 'https://api.listup.ng/api'}/advertisements/${currentAd.id}/click`
      );
