...
import { getStoreListings, Vendor, VendorListing, StoreListingsResponse } from "@/lib/api/vendors";
...
export default function StoreProfilePage() {
  ...
  const storeNameRaw = params.storeName as string;
  const storeName = decodeURIComponent(storeNameRaw).replace(/-/g, ' ');
  ...
  useEffect(() => {
    fetchStoreData();
    loadFavourites();
  }, [storeName, pagination.page, sortBy]);
  ...
  const fetchStoreData = async () => {
    try {
      setLoading(true);
      const data = await getStoreListings(storeName, pagination.page, pagination.limit, sortBy);
            ...
            <div className="flex flex-col md:flex-row h-full">
              ...
              <div className="w-full md:w-1/2 p-8 flex flex-col">
                ...
                <div className="flex items-center gap-2 text-slate-500 font-bold mb-6 text-sm">
                  ...
                  {quickViewProduct.location || "EKSU CAMPUS"}
                </div>
