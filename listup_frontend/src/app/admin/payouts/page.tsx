...
import { Loader2, Lock, CheckCircle, DollarSign, Users, Calendar } from 'lucide-react';
...
export default function AdminPayoutsPage() {
    ...
    const fetchPeriods = async () => {
        try {
            const response = await fetch('/api/admin/payouts/periods', {
                credentials: 'include',
            });
            ...
            toast.error('Failed to load payout periods');
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchStatements = async (periodId: string) => {
        try {
            const response = await fetch(`/api/admin/payouts/statements/${periodId}`, {
                credentials: 'include',
            });
    ...
    const lockMonth = async () => {
        ...
        try {
            const response = await fetch('/api/admin/payouts/lock-month', {
                method: 'POST',
                credentials: 'include',
            ...
            toast.success('Month locked and statements generated!');
            fetchPeriods();
        } catch (error) {
    ...
    const approveStatement = async (statementId: string) => {
        setActionLoading(true);
        try {
            const response = await fetch(`/api/admin/payouts/statements/${statementId}/approve`, {
                method: 'PATCH',
                credentials: 'include',
            });
    ...
    const markPaid = async (statementId: string) => {
        ...
        setActionLoading(true);
        try {
            const response = await fetch(`/api/admin/payouts/statements/${statementId}/mark-paid`, {
                method: 'PATCH',
                credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
