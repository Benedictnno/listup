...
exports.firebaseGoogleAuth = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      // 3. New user — create as USER role
      isNewUser = true;

      // 4. Check for phone number conflict before creation
      if (phone_number) {
        const existingPhone = await prisma.user.findUnique({
          where: { phone: phone_number }
        });
        if (existingPhone) {
          console.warn(`[FirebaseAuth] Phone conflict for new Google user ${email}: ${phone_number} already belongs to ${existingPhone.email}`);
          return res.status(409).json({
            success: false,
            message: 'This phone number is already linked to another account. Please login with your email and password, or use a different Google account.'
          });
        }
      }
      ...
      try {
        user = await prisma.user.create({
          data: {
            ...
            phone: phone_number || null, // Capture phone if Google provides it
        ...
        if (error.code === 'P2002') {
          console.error('[FirebaseAuth] Unique constraint violation during creation:', error.meta);
          return res.status(409).json({ 
            success: false, 
      ...
      try {
        await addToGoogleSheet(user.name, '', user.email, user.phone || '', 'USER (Google Sign-In)');
      } catch (sheetError) {
